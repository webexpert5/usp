...
class wcff {
	...
	function init() {	
		...
		if( method_exists( $this, "wcff_front_end_scripts" ) ){
			add_action( 'admin_enqueue_scripts',  array( $this, 'wcff_front_end_scripts' ) );	
		}
		if( is_admin() ) {			
	...
	function wcff_front_end_scripts(){
		wp_register_script( 'wcff-script', wcff()->info['dir'] . "assets/js/wcff.js", 'jquery', wcff()->info['version'] );
		wp_register_style( 'wcff-style', wcff()->info['dir'] . 'assets/css/wcff.css' );
		wp_enqueue_style( 'wcff-script' );
	...
	function wcff_includes() {		
		...
		include_once('classes/fields/wcff-url.php');
		include_once('classes/wcff-cart-updater.php');
