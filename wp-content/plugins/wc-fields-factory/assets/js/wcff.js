...
(function($) {	
	...
	var wcff = function() {
		...
		this.registerEvents = function() {			
			...
			$(document).on( "click", ".wcff-date-field-disable-accordian-left-panel li", this, function(e) {	
				var clickval = $( this );
				
				$( ".wcff-date-field-disable-accordian-left-panel li" ).removeClass( "active" );
				clickval.addClass( "active" );
				
				$( ".wcff-date-field-disable-accordian-right-panel div.wcff-date-field-disable-hide-elem" ).hide();
				$( "div"+clickval.data( "box" ) ).show();
			});		
			$(document).on( "click", ".wcff-date-disable-radio-clear", this, function(e) {	
				$( "input[name=wcff-field-type-meta-weekend_weekdays]" ).prop( "checked", false );
				e.preventDefault();
			});
			...
			$(document).on( "change", "#check_single_multi", this, function(e){
				if( $( "#check_single_multi" ).length != 0 ) {
					e.data.multiple_admin_checkbox( $( "#check_single_multi" ).prop( "checked" ) );				
				}
			});			
		};
		
		this.multiple_admin_checkbox = function( _con ){		
			if( _con ){
				$( "div.wffaf-multi-container" ).show();
				$( "div.wffaf-single-container" ).hide();
			} else {
				$( "div.wffaf-multi-container" ).hide();
				$( "div.wffaf-single-container" ).show();
			}
		};
		...
		this.renderSingleView = function( _target ) {
			...
			this.multiple_admin_checkbox( $( "#check_single_multi" ).prop( "checked" ) );
		};
