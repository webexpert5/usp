...
class wcff_field_email extends wcff_field {
	...
	function render_product_field( $field ) { 		
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<?php else: ?>
				<input type="email" class="wccpf-field <?php echo $field_class; ?>" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" wccpf-type="email" wccpf-pattern="email" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> value="<?php echo $field["default_value"]; ?>" />
