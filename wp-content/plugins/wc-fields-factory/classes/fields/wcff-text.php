	...
	function render_product_field( $field ) { 
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<?php if( $show_value_front == "yes" ) : ?>		
				<p><?php echo esc_attr( $field["default_value"] ); ?></p>
			<?php else: ?>
				<input type="text" class="wccpf-field<?php echo $field_class; ?>" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" value="<?php echo esc_attr( $field["default_value"] ); ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" maxlength="<?php echo esc_attr( $field["maxlength"] ); ?>" wccpf-type="text" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> />
