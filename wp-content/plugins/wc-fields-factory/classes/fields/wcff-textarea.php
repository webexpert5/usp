 ...
 * Class which responsible for creating and maintaining textarea field ( for both Product, Admin, as well as textarea fields's meta section )
 */
...
class wcff_field_textarea extends wcff_field {
	...
	function render_product_field( $field ) { 
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<?php else: ?>
				<textarea class="wccpf-field<?php echo $field_class; ?>" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" rows="<?php echo esc_attr( $field["rows"] ); ?>" maxlength="<?php echo esc_attr( $field["maxlength"] ); ?>" wccpf-type="text" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> ><?php echo esc_html( $field["default_value"] ); ?></textarea>
