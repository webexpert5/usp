 ...
 * Class which responsible for creating and maintaining radio field ( for both Product, Admin, as well as radio fields's meta section )
 */
	...
	function render_wcff_setup_fields( $type = "wccpf" ) { ob_start(); ?>
		...
		<tr>
			<td class="summary">
				...
				<p class="description"><?php _e( 'Enter default selected option', 'wc-fields-factory' ); ?><br/><br/>blue|Blue</p>
			</td>
			<td>
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<?php else: ?>
				...
				foreach ( $choices as $choice ) {
					...
					<li><label><input type="radio" class="wccpf-field <?php echo $field_class; ?>" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" value="<?php echo esc_attr( trim( $key_val[0] ) ); ?>" <?php echo $attr; ?> wccpf-type="radio" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> /> <?php echo esc_html( trim( $key_val[1] ) ); ?></label></li>
				<?php } ?>		
				</ul>
