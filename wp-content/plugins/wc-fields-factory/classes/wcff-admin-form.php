...
class wcff_admin_form {
			...
			foreach ( $all_fields as $title => $fields ) {
				if( count( $fields ) > 0 ) {
					foreach ( $fields as $key => $field ) {		
						...
						if( $is_multi_admin && $field[ "type" ] == "checkbox" && $post->post_type == "product" ) {							
							$check_count = explode(";", $field["choices"] );
							$check_new_obj = array();
							for ( $i = 0; $i <  sizeof( $check_count ); $i++ ){
								$explode_val = explode( "|", $check_count[ $i ] );
								if( isset( $_REQUEST[ "wccaf-field-". $explode_val[0] ] ) ) {
									$check_new_obj[ "wccaf_". $explode_val[0] ] = $_REQUEST[ "wccaf-field-". $explode_val[0] ];
								}
							}							
							update_post_meta( $post_id, "wccaf_" . $field["name"], wp_slash( json_encode( $check_new_obj ) ) );								
	...
	function add_color_picker_styles_scripts(){
	...
	function wccaf_back_end_enqueue_scripts( $is_color_picker_there, $is_image_field_there ) {
		if( $is_color_picker_there ) {
			add_action( 'wp_enqueue_scripts', 'add_color_picker_styles_scripts' );			
		}
