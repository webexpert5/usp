		...
		public static function productConfigurationForm(){
			...
			<div id="catalogs_upload">
					...
					<!--<div class="box-items">-->
						...
						<input type="file" name="async-upload" class="image-file" accept="image/*" />
					...
					<input type="hidden" id="front_image_id" name="option_id-cd_front_image" value="" />	
					...
					/*$args = array(
					    ...
					    "allowed_extensions" => "jpg, png, bmp, gif, jpeg",
			...
			if(isset($value['variation_id']) && $value['variation_id']!=0){
			   ...
			   foreach($value as $subkey=>$sub_value){
				   if (preg_match('/(option_id-*)/',$subkey)){
					   if($subkey=='option_id-front_image' || $subkey=='option_id-back_image' || $subkey=='option_id-proof'){
						     continue;      
						}
